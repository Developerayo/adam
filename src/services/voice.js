...
async function checkSoxInstallation() {
  try {
    await execAsync('sox --version', { stdio: 'ignore' })
    return true
  } catch (error) {
...
async function handleNoSoxChoice() {
  let action = 'continueWithTextInput'
...
async function handleNoWhisperChoice() {
  let action = 'continueWithTextInput'
...
async function installSox() {
  try {
    if (platform === 'win32') {
      ...
      await execAsync('brew install sox', { stdio: 'inherit' })
    } else if (platform === 'linux') {
      console.log('Installing sox...')
      await execAsync('sudo apt-get update', { stdio: 'inherit' })
      await execAsync('sudo apt-get install -y sox libsox-fmt-all', { stdio: 'inherit' })
    } else {
      console.error(chalk.red('We do not recognize this OS. Please install Sox manually.'))
      process.exit(1)
    }
  } catch (err) {
    console.error(chalk.red(`Failed to install Sox: ${err.message}`))
    process.exit(1)
  }
}
async function installWhisper() {
  ...
  try {
    ...
    console.error('Failed to download whisper.cpp binary.', err)
    process.exit(1)
  }
...
const recordAudio = async () => {
  ...
  await Promise.race([
    ...
    new Promise(resolve => recording.on('close', resolve)),
  ])
