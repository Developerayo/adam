...
const projectDirMap: Record<ProjectType, string[]> = {
	nodejs: ['package.json'],
...
async function doesFileExist(cwd: string, file: string): Promise<boolean> {
	if (file.includes('*')) {
...
export async function getGitInfo(cwd: string, isFullInfoNeeded: boolean = false): Promise<GitInfo> {
	try {
		...
		const changes = status && status ? status.split('\n').filter(line => line.trim() !== '') : []
...
export function fetchDependencyCount(jsonStruct: CwdStructure): DependencyCount | null {
...
export async function analyzeCwd(
	cwd: string,
	isCommitRelated: boolean = false,
): Promise<CwdStructure> {
	const files = await fs.readdir(cwd)
	...
	for (const [projectType, typeFiles] of Object.entries(projectDirMap)) {
		try {
			const exists = await Promise.any(typeFiles.map(file => doesFileExist(cwd, file)))
			if (exists) {
				detectedTypes.push(projectType as ProjectType)
			}
		} catch {
			// None of the files exist for this project type
		}
	}
